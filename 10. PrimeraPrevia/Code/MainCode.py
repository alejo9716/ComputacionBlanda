# @Autores: Edisson Montes, Alejandro Rios
#!/usr/bin/env python
# coding: utf-8
...
# Datos de trabajo
# -----------------------------------------------------------------
data = np.genfromtxt(os.path.join(DATA_DIR, "Datos.tsv"), 
                     delimiter="\t")
...
def plot_models(x, y, models, fname, mx=None, ymax=None, xmin=None):
    ...
    # En el primer corchete se tiene: 1*365 + 2*365 + ..., hasta
    ...
    # cuales se agrupan en paquetes de w*365. Esto permite
    # determinar los valores de w desde 0 hasta 30, indicando
    # con ello que se tiene un poco más de 30 años
    
    # Estos valores se utilizan en el segundo corchete para
    # escribir las etiquetas basadas en estos valores de w
    
    # Por tanto, se escriben etiquetas para w desde 0 hasta
    # 30, lo cual constituye los años analizadas a lo largo del tiempo
    plt.xticks(
        [w * 365 for w in range(30)], 
...
# de inflexión
# -----------------------------------------------------------------
#calcula el punto de inflexion en el año 15 
inflexion =  15 * 365
...
# limites(con proyeccion a futuro) especificamente hasta el año 24
...
# se construyen nuevamente los modelos de 1,2,3,10,50 dimensiones pero
...
# especificamente hasta el año 24 en la ruta y con el nombre de archivo indicado
plot_models(
...
fbt100 = sp.poly1d(np.polyfit(xb[train], yb[train], 50))
...
# precisamente hasta el año 24,con la ruta y el nombre de archivo indicado  
plot_models(
...
# nota2= se divide el resultado obtenido en dias (365)
# para obtener el resultado dado en años

print("\nPrediccion realizada a partir de totos los datos")
alcanzado_max2 = fsolve(f2 - 4500, x0=7575) / (365)
print("VALOR DOLAR $4500 ESPERADO EN EL AÑO %f" % 
      alcanzado_max2[0])

print("\nPrediccion realizada solo segun los datos siguientes al 2015")
alcanzado_max = fsolve(fbt2 - 4500, x0=7575) / (365)
print("VALOR DOLAR $4500 ESPERADO EN EL AÑO %f" % 
      alcanzado_max[0])
